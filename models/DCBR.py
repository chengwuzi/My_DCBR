...
class DCBR(nn.Module):
    ...
    def get_propagation_graph(self, bipartite_graph):
        ...
        rowsum_sqrt = sp.diags(1/(np.sqrt(np.array(propagation_graph.sum(axis=1)).ravel()) + 1e-8))
        colsum_sqrt = sp.diags(1/(np.sqrt(np.array(propagation_graph.sum(axis=0)).ravel()) + 1e-8))
    ...
    def get_aggregation_graph(self, bipartite_graph):
        ...
        bipartite_graph = sp.diags(1/np.array(bundle_size).ravel()) @ bipartite_graph
